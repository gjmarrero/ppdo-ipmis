...
const handleOfficeFormSubmit = async () => {
    loading.value = true
    try {
        let response
        if (props.officeToEdit) {
            response = await api.put(`/api/office/${props.officeToEdit.id}`, officeForm)
        } else {
            response = await api.post('/api/office', officeForm)
        }
...
const handleUserFormSubmit = async () => {
    ...
    try {
        ...
        emit('update:isSettingAddDialogOpen', false)
        emit('settingAdded', response.data.data)
    } catch (error) {
        console.error('Save failed:', error)
    } finally {
