...
watch(() => props.modelValue, (newValue) => {
  if (newValue) {
    query.value = newValue.name // Set default value in input field
  } else {
    query.value = '' // 👈 clear input if modelValue is null/empty
  }
...
const selectOption = (option) => {
  query.value = option.name // Update input field with selected option
...
<template>
  <div class="w-full mt-2">
    <!-- Search Input -->
    <input v-model="query" @focus="isOpen = true" @blur="handleBlur" @keydown.arrow-down.prevent="moveSelection(1)"
      @keydown.arrow-up.prevent="moveSelection(-1)"
      @keydown.enter.prevent="selectOption(filteredOptions[highlightedIndex])" @keydown.esc="closeDropdown"
      class="w-full h-10 border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500"
      :placeholder="placeholder" :disabled="disabled" />

    <!-- Dropdown List -->
    <ul v-if="isOpen && filteredOptions.length" class="absolute z-10 mt-1 w-64 bg-white border rounded-lg shadow-lg">
