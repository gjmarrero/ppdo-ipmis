...
export function getBadgeVariant(value: string, type: 'validation_status' | 'preengineering_status' | 'qcp_status'): string {
...
export const getPreEngColumns = ({
    ...
    onRefresh: () => void,
        ...
        },
        {
            ...
            cell: ({ row }: any) => {
                const rawStatus = row.getValue('preengineering_status')
                const status = rawStatus ?? 'For pre-engineering'
                const badgeVariant = getBadgeVariant(status, 'preengineering_status')
                return h(
                    Badge,
                    {
                        variant: badgeVariant,
                        class: 'uppercase items-center justify-center text-center',
                        style: 'border-radius: 10px 60px 30px'
                    },
                    () => status
                )
