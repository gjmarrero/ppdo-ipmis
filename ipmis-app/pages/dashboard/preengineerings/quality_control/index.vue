<template>
    ...
    <div v-else class="flex min-h-screen w-full flex-col bg-mainbg px-4 py-4">
        ...
        <PreEngineeringQualityControlAdd v-if="isPreEngFormDrawerOpen" v-model="isPreEngFormDrawerOpen" :mode="mode"
        ...
        <ClientOnly>
            <PreEngineeringDetails v-if="isDetailDrawerOpen" v-model:isOpen="isDetailDrawerOpen"
                :funded_project="selectedProject" :key="selectedProject?.id" @update-qcp="openEditPreEng"
                @add-preengineering="openAddPreEng" @open-activity-dialog="openAddActivityDialog"
                @manage-other-requirements="openManageOtherRequirements" :showPow="false" :showQc="true" :showReview="false"/>
        </ClientOnly>
...
const openAddPreEng = () => {
    ...
    isPreEngFormDrawerOpen.value = true
    console.log("add preeng for this project", mode.value)
}
...
const handleFormSubmission = (mode) => {
    ...
    toast({
        title: 'Success',
        description: 'QCP status updated'
    })
    refreshDetailDrawer()
}
...
onMounted(() => {
    fetchPreEngineerings()
    applyFilter()
    console.log("preengineering", preEngineerings)
})
