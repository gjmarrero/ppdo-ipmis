<template>
    ...
    <div v-else class="flex min-h-screen w-full flex-col bg-mainbg px-4 py-4">
        ...
        <PreEngineeringReviewAdd v-if="isPreEngFormDrawerOpen" v-model="isPreEngFormDrawerOpen" :mode="mode"
        ...
        <ClientOnly>
            <PreEngineeringDetails v-if="isDetailDrawerOpen" v-model:isOpen="isDetailDrawerOpen"
                :funded_project="selectedProject" :key="selectedProject?.id" @update-review="openEditPreEng"
                @add-preengineering="openAddPreEng" @open-activity-dialog="openAddActivityDialog"
                @manage-other-requirements="openManageOtherRequirements" :showPow="false" :showQc="false" :showReview="true" />
        </ClientOnly>
